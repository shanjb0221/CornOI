...
template<class T> struct linkNode {
	...
	T* to;
	linkNode* nt;
	linkNode(T* t,linkNode* n,char c):to(t),nt(n),ch(c) {};
};
int cnt;
struct trie1Node {
	...
	int siz,dfn;
	int idx;
	trie1Node():h(NULL),siz(1),dfn(0),idx(++cnt) {};
};
struct trie0Node {
...
template<class T>struct trie {
	...
	inline T* findLink(T* pos,char v) {
		for(linkNode<T>* i=pos->h; i; i=i->nt)
			if(i->ch==v)return i->to;
		return NULL;
	}
	inline void addLink(T* pos,T* dst,char v) {
		pos->h=new linkNode<T>(dst,pos->h,v);
	}
